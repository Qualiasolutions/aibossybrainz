			...
			<div className="relative flex h-full flex-col p-3">
				...
				>
					<div className="flex items-center gap-2">
						...
						<span className="font-semibold text-white/90 text-xs">Boss Brainz</span>
					</div>
								...
								<div className="flex gap-2">
									...
									<div className="space-y-1.5">
										...
										{phase >= 4 && (
											...
											>
												{[
													{ icon: TrendingUp, label: "Growth", color: "emerald" },
													...
													>
														<item.icon className={`size-2.5 text-${item.color}-400`} />
														<span className={`text-${item.color}-300 text-[9px] font-medium`}>
															{item.label}
														</span>
													</motion.div>
												))}
				...
				>
					<div className="flex-1">
						{phase === 1 ? (
							<motion.div className="flex items-center">
								...
								>
									{userMessage.slice(0, Math.floor((phase === 1 ? 1 : 0) * userMessage.length))}
								</motion.span>
								<motion.span
							...
							<span className="text-white/30 text-[11px]">Ask anything about your business...</span>
						) : (
							<span className="text-white/30 text-[11px]">Type your message...</span>
						)}
					</div>
					<motion.div
					...
					>
						<Send className={`size-3 ${phase >= 1 ? "text-white" : "text-white/40"}`} />
					</motion.div>
				</motion.div>
			</div>
			...
			<AnimatePresence>
				{phase >= 4 && (
					<>
						...
						>
							<span className="font-medium text-emerald-300 text-[9px]">24/7 Available</span>
						</motion.div>
						<motion.div
						...
						>
							<span className="font-medium text-purple-300 text-[9px]">Expert Strategies</span>
						</motion.div>
					</>
				)}
			</AnimatePresence>
