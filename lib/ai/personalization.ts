	...
	const stageContent = stages
		.map((stage) => {
			...
			return `${stage}: ${items
				...
				.map(
					(t) =>
						`${t.content}${t.type !== "touchpoint" ? ` (${t.type})` : ""}`,
				)
				.join("; ")}`;
		})
		.filter(Boolean);
...
export async function buildPersonalizationContext(
	...
	try {
		...
		if (profile) {
			const parts = [];
			if (profile.displayName) parts.push(`User's name is ${profile.displayName}`);
		...
		if (swot?.data) canvasSections.push(formatSwotForContext(swot.data as unknown as SwotData));
		if (bmc?.data) canvasSections.push(formatBmcForContext(bmc.data as unknown as BusinessModelData));
		if (journey?.data) {
